	...
	// This address must be verified with Amazon SES.
		...
		final String PASSWORD_RESET_HTMLBODY = "<h1>A request to reset your password</h1>"
			  ...
			  final String PASSWORD_RESET_TEXTBODY = "A request to reset your password "
				 ...
				 * AmazonSimpleEmailServiceClientBuilder.standard().withRegion(Regions.
				 * US_EAST_1) .build();
				 * 
				 * String htmlBodyWithToken = HTMLBODY.replace("$tokenValue",
				 * userDto.getEmailVerificationToken()); String textBodyWithToken =
				 * TEXTBODY.replace("$tokenValue", userDto.getEmailVerificationToken());
				 * 
				 * SendEmailRequest request = new SendEmailRequest() .withDestination(new
				 * Destination().withToAddresses(userDto.getEmail())) .withMessage(new Message()
				 * .withBody(new Body().withHtml(new
				 * Content().withCharset("UTF-8").withData(htmlBodyWithToken)) .withText(new
				 * Content().withCharset("UTF-8").withData(textBodyWithToken))) .withSubject(new
